...
happy %>% filter(!is.na(happy)) %>% group_by(happy,marital) %>% arrange(marital)%>% group_by(marital,happy) %>% summarise(cont=n()) %>% drop_na() %>%
  ...
  labs( x="Estado civil", y ="Cantidad", title="Gráfico de barras apiladas de estado civil y nivel de felicidad") +
...
happy %>% filter(!is.na(happy)) %>% group_by(happy,marital) %>% arrange(marital)%>% group_by(marital,happy) %>% summarise(cont=n()) %>% drop_na() %>%
  ...
  labs( x="Estado civil", y ="Proporción", title=" Gráfico de barras al 100% apiladas de estado civil y nivel de felicidad") +
  theme_minimal( base_size=11) +
  theme( plot.title = element_text( hjust = 0.5, colour="darkslategrey", size=13.7, face="plain")) +
  scale_fill_manual( values=c("firebrick3","seagreen3","royalblue4"))
```
...
Un mosaico es el análogo gráfico de una tabla de contingencias.
...
happy%>%
  ...
  spread(happy, n)%>%
  kable()
#Acá estaá la tabla de contingencias con los totales pero hay que cambiar el tamanño de la fuente para que salga bien en la slide

# library(gmodels)
# with(happy, CrossTable(marital, happy,max.width = 1,prop.r=FALSE,prop.c = FALSE,prop.chisq = FALSE,prop.t = FALSE))
...
happy %>% filter(!is.na(happy)) %>% group_by(happy,marital) %>% arrange(marital)%>% group_by(marital,happy) %>% summarise(cont=n()) %>% drop_na() %>%
  ...
  labs( x="Estado civil", y="Porcentaje", title="Gráfico de mosaicos del estado civil y felicidad") +
  theme_minimal( base_size=11) +
...
Una bondad importante de este diagrama es su aporte probabilísticio, dado que el área de cada mosaico corresponde a la probabilidad condicionada (P(y=A/x=B) cumplen A nivel de *y* dado que cumplen B nivel de *x*). 

- Ejemplo: ¿Cuál es la probabilidad de ser muy feliz dado que estoy casado? $P(y="very happy"/ x="married")$

- El área de el mosaico donde se intersectan las condiciones representa la probabilidad condicional antes mencionada.
<!-- - Acaá teneés que saber el concepto de probabilidad condicional P(A/B)=P(A,B)/P(B) -->
<!-- no te preocupes yo lo puedo aclarar en clase si hace falta -->
...
<!-- ```{r,echo=FALSE} -->
<!-- round(prop.table(table(happy$marital)),2)[1]*round(prop.table(table(happy$happy)),2)[3] -->
<!-- ``` -->
...
- Pueden ser difíciles de interpretar, sobre todo con una gran cantidad de niveles.

- Difícil hacer comparaciones con precisión entre cada segmento.
...
data=as.data.frame.table(apply(Titanic, c(2:4), sum))
 ggplot(data) + 
  geom_mosaic(aes(weight=Freq, x=product(Survived,Age,Sex),fill=Sex))
```
